...
- **TEMPORARY IMMEDIATE NOTIFICATION COMPLETION HMAC EXCEPTION (owner-approved 2026-09-25 in the PR 3468 follow-up; expires when HMAC provisioning moves to a restricted worker role or on 2026-10-09, whichever comes first):** only the CRON_SECRET-authenticated call graph rooted at `apps/web/src/app/api/cron/provision-immediate-notification-completion-hmac/route.ts` may construct the branded client through `apps/web/src/lib/immediate-order/server-completion-hmac-client.ts`, and only after authentication succeeds. The privileged client is limited to the exact `set_immediate_notification_completion_hmac_secret` RPC. The helper, route, and provisioner are server-only and must never enter a client graph. No sibling route, generic service-role operation, table query, additional RPC, importer, or re-export inherits authorization. The completion HMAC secret, service-role material, and provider bodies must never enter responses, logs, event payloads, or client bundles. The backend key inherently bypasses RLS; these controls constrain the approved call graph, not that capability. Remove the exception when the restricted worker role is available, or obtain explicit reapproval by the fixed expiry.
