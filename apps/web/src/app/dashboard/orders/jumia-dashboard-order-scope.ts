 ...
 * does not belong to the merchant, so callers fail closed. Throws when
 * the lookup itself fails so callers surface the failed query instead
 * of rendering a successful empty list.
 *
...
export async function resolveJumiaDashboardOrderScope(
  ...
  if (error) {
    logger.error({
      message: 'Error resolving Jumia dashboard order scope',
      error,
      merchantId,
      integrationId,
      route: 'dashboard/orders/getOrders',
    });
    throw new Error('Failed to resolve Jumia dashboard order scope');
  }
