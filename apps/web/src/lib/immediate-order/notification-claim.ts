...
export interface ImmediateNotificationCompletionProbe {
   ...
   * secret is unprovisioned (or the probe errored): the caller must
   * skip the send without completing — the never-started claim expires
   * on its short grace and a later replay resumes delivery.
   */
  provisioned: boolean;
 ...
 * after() sends. Releases our own fresh claim through the proof-bound
 * failed path, then immediately reclaims it: a provisioned secret
 * releases failed claims instantly, so the reclaim re-wins with a
 * fresh lease; an unprovisioned secret no-ops the release, so our own
 * fresh lock blocks the reclaim and the probe observably loses. No new
 * RPC or oracle surface — the verdict comes from the existing
 * claim/complete protocol. Never rejects: any probe failure reads as
 * unprovisioned (delivery deferred to replay, never sent
 * uncompletable).
 */
export async function probeImmediateNotificationCompletionProvisioned(
  ...
  try {
    ...
    if (error) {
      logger.error({
        message:
          'Immediate order notification provisioning probe failed; deferring delivery to replay',
        error,
        orderId,
      });
      return unprovisioned;
    ...
    logger.error({
      message:
        'Immediate order notification provisioning probe raised; deferring delivery to replay',
      error,
      orderId,
    });
    return unprovisioned;
