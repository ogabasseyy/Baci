...
/**
 * Sole export: priority lane for shell-critical tiny reads (navigation
 * categories: ~19 rows, <2KB, <10ms indexed). Same timeout budget as the
 * shared transport, but NEVER the build semaphore: during prerender these
 * reads must land in the static shell instead of queueing behind bulk
 * reads until the prerender aborts (which shipped 1246 blog posts with an
 * empty nav). The semaphore guards worker OOM from concurrent BULK reads;
 * a millisecond tiny read cannot accumulate, so bypassing it changes no
 * hazard envelope.
 */
