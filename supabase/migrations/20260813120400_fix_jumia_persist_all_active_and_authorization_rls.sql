-- Persist rotated credentials even when every selected marketplace is already active,
-- and let integrations.manage staff read stored authorization grants.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    SET
      jumia_authorization_id = v_authorization_id,
      updated_at = now()
    WHERE integration.merchant_id = p_merchant_id
      ...
      AND EXISTS (
        SELECT 1
        FROM unnest(p_shop_ids, p_marketplace_labels) AS selected(shop_id, marketplace_key)
        WHERE integration.shop_id = btrim(selected.shop_id)
          AND integration.marketplace_key = btrim(selected.marketplace_key)
      );
...
DROP POLICY IF EXISTS jumia_authorizations_select_policy ON public.jumia_authorizations;
