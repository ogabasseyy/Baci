-- Restore the manage-only boundary after the temporary view-permission experiment proved unsafe.
-- View-authorized staff must refresh through trusted server paths that call
-- these RPCs with the service role after API-route authorization.
