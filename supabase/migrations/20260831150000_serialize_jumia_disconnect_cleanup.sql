-- Serialize disconnect cleanup by both provider shop and shared authorization.
-- A shared authorization can back several shops, so locking only the shop
-- allows concurrent disconnects to miss the last reference.
...
BEGIN
  ...
  INTO v_shop_id, v_authorization_id
  ...
  IF v_shop_id IS NULL OR v_authorization_id IS NULL THEN RETURN; END IF;
  ...
  IF v_authorization_id IS NULL THEN RETURN; END IF;
