-- Wallet reservations must consume a quote attested by the trusted Admin
-- quote writer. Authenticated callers can create/update ordinary quotes, so
-- quote economics alone are not sufficient authorization for a debit.
...
BEGIN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'merchant-shipping:' || v_order.merchant_id || ':' || p_order_id,
      0
    )
  );
  ...
  WHERE order_id = p_order_id
    AND shipping_quote_id = p_quote_id
  FOR UPDATE;

  -- Existing reservations are idempotent even after the provider quote
  -- expires. New reservations still require an active quote below.
  SELECT * INTO v_quote
  ...
  IF v_quote.id IS NULL
     OR v_attestation.quote_id IS NULL
  ...
  INSERT INTO public.merchant_shipping_charges(
