-- Checkout shipping retention must not follow a later selected_quote_id
-- swap. Stamp freezes the customer-checkout snapshot after first write, and
-- GIGL settlement reads only that order snapshot — never the live quote price.
