-- Finalize merchant-wallet DVA assignment only for the request locked by this
-- transaction. A provider callback can arrive after a request timeout (or
-- after a newer request has replaced it), so a caller-side pending precheck is
-- not sufficient. Fulfilled requests retain an exact, idempotent replay path.
...
BEGIN
  ...
  -- This row lock serializes timeout/failure transitions with the provider
  -- callback. Never trust a pending state observed by the caller.
  ...
  -- Failed and expired requests (and any future terminal state) can never be
  -- resurrected by a delayed provider assignment.
  ...
  -- Guard the terminal transition as well; if a concurrent lifecycle change
  -- ever wins the lock before this point, roll back the account assignment.
