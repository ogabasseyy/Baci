-- Wallet-funded GIGL reservations spend the merchant owner's wallet. Staff may
-- still edit or fulfill orders, but may not authorize a new wallet debit.
-- Keep the order mutex and row lock before the owner check so authorization,
-- cancellation, and reservation observe one serialized lifecycle state.
