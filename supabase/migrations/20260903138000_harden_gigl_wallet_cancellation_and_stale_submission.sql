-- Block cancellation while a wallet charge is booked but the order row has not
-- yet caught up, and recover stale provider submissions so retries can finish
-- charge completion instead of wedging on SHIPMENT_BOOKING_IN_PROGRESS forever.
