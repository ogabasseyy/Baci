-- Restore wallet-account persistence to the service-role boundary. Merchant
-- recovery completes through a server-side Paystack probe that uses the same
-- service-role persist path as assignment webhooks. Keep booked-charge
-- rebinding protections from the prior migration.
