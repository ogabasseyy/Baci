-- Allow recovery of a non-stale provider_submitting charge when a shipment was
-- already persisted: refresh the attempt token so completion can finish.
...
BEGIN
  ...
  IF v_existing.id IS NOT NULL THEN
    ...
    ELSIF v_existing.status = 'provider_submitting' THEN
      UPDATE public.merchant_shipping_charges
      SET attempt_token_digest = pg_catalog.encode(extensions.digest(p_attempt_token, 'sha256'), 'hex'),
          updated_at = now()
      WHERE id = v_existing.id;
