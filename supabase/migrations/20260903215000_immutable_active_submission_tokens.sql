-- Keep active provider_submitting tokens immutable. Token rotation is only
-- allowed inside a recovery boundary that first verifies a matching shipment
-- and completes the charge in the same transaction.
...
BEGIN
  ...
  IF v_charge.status = 'booked'
     AND v_charge.shipment_id IS NOT DISTINCT FROM v_shipment.id THEN
    RETURN v_charge.status;
  END IF;

  IF v_charge.status IS DISTINCT FROM 'provider_submitting' THEN
    RAISE EXCEPTION 'charge_not_recoverable' USING ERRCODE = '22023';
  END IF;

  UPDATE public.merchant_shipping_charges
  SET attempt_token_digest = v_digest,
      status = 'booked',
...
REVOKE ALL ON FUNCTION public.recover_merchant_shipping_charge_for_persisted_shipment(
  uuid, text, uuid
) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.recover_merchant_shipping_charge_for_persisted_shipment(
  uuid, text, uuid
) TO authenticated;
