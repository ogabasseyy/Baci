-- Re-assert immutable attempt tokens for active provider submissions and block
-- authenticated refunds while a non-stale provider submission is in flight.
...
BEGIN
  ...
  IF v_charge.status = 'provider_submitting'
     ...
     AND v_charge.provider_submitting_at > now() - interval '15 minutes' THEN
