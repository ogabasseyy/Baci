-- Select the unpaid pickup identified by a resume claim when p_repair_id is
-- provided. Email-only newest lookup remains available when p_repair_id is null
-- (legacy callers); storefront resume always passes the claim repair id so a
-- newer unpaid sibling cannot steal the reclaim.

DROP FUNCTION IF EXISTS public.find_resumable_repair_pickup(uuid, text);
...
COMMENT ON FUNCTION public.find_resumable_repair_pickup(uuid, text, uuid) IS
  'Server-capability-only unpaid pickup repair reclaim; optional p_repair_id pins the resume claim ticket.';
