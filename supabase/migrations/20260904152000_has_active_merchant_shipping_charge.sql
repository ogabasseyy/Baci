-- Staff-authorized projection for active wallet shipping reservations.
-- Authenticated clients must not rely on merchant_shipping_charges table RLS
-- alone: staff retries need the same fulfill/edit gate as reserve RPCs.
...
COMMENT ON FUNCTION public.has_active_merchant_shipping_charge(uuid, uuid)
  IS 'Returns true when an authorized caller has an active reserved/provider_submitting wallet shipping charge for the order quote.';
