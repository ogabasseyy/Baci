-- Clear pickup_payment_pending_reference on successful confirm and when a
-- trusted mismatch is ledgered for review.
...
BEGIN
  ...
  SET pickup_payment_status = CASE
      ...
      pickup_payment_pending_reference = NULL,
      pickup_fee = p_amount,
      pickup_currency = 'NGN',
      pickup_paid_at = now()
  WHERE repair.id = p_repair_id
...
COMMENT ON FUNCTION public.confirm_repair_pickup_payment(
  ...
  'Atomically records a verified customer-funded repair pickup payment, clears any pending Paystack reference binding, and includes late captures after the repair became terminal. Service-role webhook only; duplicate references are idempotent.';
...
COMMENT ON FUNCTION public.record_repair_pickup_payment_mismatch(
  ...
  'Atomically ledgers a verified Paystack repair-pickup charge that failed claim validation, marks the repair for review, and clears any pending Paystack reference binding. Service-role webhook only; duplicate references are idempotent.';
