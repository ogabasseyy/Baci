-- Keep merchant manual fulfillment (and booked) terminal when a late Paystack
-- capture settles. Still ledger the money and clear matching pending refs, but
-- never overwrite those statuses to paid (which would re-open GIGL booking).
...
COMMENT ON FUNCTION public.confirm_repair_pickup_payment(
  ...
  'Atomically records a verified customer-funded repair pickup payment, consumes the matching pending reference history row, clears the tip when it matches, preserves manual_fulfilled/booked on late capture, and includes late captures after the repair became terminal. Service-role webhook only; duplicate references are idempotent.';
