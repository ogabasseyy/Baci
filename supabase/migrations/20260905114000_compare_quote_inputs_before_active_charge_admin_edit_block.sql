-- Only reject admin edits while a wallet shipping charge is active when the
-- submitted payload would change quote inputs (shipping_address, items, or
-- shipping_fee). Notes/customer-only/no-op edits must still succeed during
-- long-lived reconciliation holds.
