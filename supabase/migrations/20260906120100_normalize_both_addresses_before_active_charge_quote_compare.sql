-- Normalize both existing and reconstructed shipping addresses before the
-- active-charge quote-input comparison so empty-string city/state keys do not
-- trip a false 409 on notes/customer-only edits.
