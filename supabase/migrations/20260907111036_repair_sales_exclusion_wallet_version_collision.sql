-- Repair the already-applied wallet migration's collision with the sales version.
-- Run early through the migration applier, before the refund follow-on replaces
-- the reconciliation function. Fresh histories already ran the sales migration.
DO $sales_collision$
BEGIN
  IF EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations
    WHERE version = '20260903120000' AND name = 'guard_merchant_wallet_paystack_dva_alias') THEN
    IF EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations
      WHERE version = '20260904110100') THEN
      RAISE EXCEPTION 'sales collision repair must precede refund follow-on';
    END IF;
    EXECUTE $sales_source$
-- Customer-funded repair pickup fees are pass-through logistics charges, not
-- merchant order revenue. Keep them out of merchant sales and admin capture
-- totals while preserving the underlying transaction rows for audit/refund.
CREATE OR REPLACE FUNCTION public.is_merchant_sales_transaction(
  p_metadata jsonb
)
RETURNS boolean
LANGUAGE sql
...
SET search_path = ''
AS $$
  SELECT COALESCE(p_metadata ->> 'transaction_type', '')
    IS DISTINCT FROM 'repair_pickup';
$$;
...
COMMENT ON FUNCTION public.is_merchant_sales_transaction(jsonb) IS
  'Returns false for customer-funded repair pickup fee captures so pass-through logistics charges do not inflate merchant sales totals.';
$sales_source$;
  END IF;
END;
$sales_collision$;
