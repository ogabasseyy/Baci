...
-- Keep case-insensitive lookup for mixed-case callers while comparing against
-- the indexed merchants.slug column (values are normalized to lowercase).
...
BEGIN
  ...
  FROM merchants m
  WHERE m.slug = lower(trim(p_merchant_slug))
  LIMIT 1;
