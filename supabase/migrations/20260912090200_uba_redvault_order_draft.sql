...
BEGIN
  ...
  IF jsonb_typeof(v_terms) IS DISTINCT FROM 'object' OR NOT (v_terms ?& ARRAY['campaign_dates','minimum_spend','caps','usage_limits','stacking','split_payments','funding_fees','refund_usage_restoration','operations_owner'])
    OR EXISTS (SELECT 1 FROM jsonb_each(v_terms) WHERE value = 'null'::jsonb OR value = '""'::jsonb) THEN
    RAISE EXCEPTION 'redvault_commercial_terms_missing';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM public.discount_codes WHERE discount_codes.id = v_code_id AND merchant_id = v_merchant_id AND discount_type = 'percentage' AND discount_value = 5 AND is_active IS TRUE) THEN RAISE EXCEPTION 'redvault_binding_invalid'; END IF;
  ...
  SELECT created.id INTO v_order_id FROM public.create_storefront_order(
    p_merchant_id => v_merchant_id, p_customer_email => p_order ->> 'customer_email', p_customer_name => p_order ->> 'customer_name',
    p_items => COALESCE(p_order -> 'items', '[]'::jsonb), p_customer_phone => NULLIF(p_order ->> 'customer_phone', ''),
    p_shipping_fee => COALESCE((p_order ->> 'shipping_fee')::numeric, 0), p_discount_amount => v_discount_kobo::numeric / 100,
    p_tax_amount => COALESCE((p_order ->> 'tax_amount')::numeric, 0), p_payment_method => 'uba_redvault', p_payment_status => 'unpaid', p_shipping_status => 'pending',
    p_shipping_address => p_order -> 'shipping_address', p_source => COALESCE(NULLIF(p_order ->> 'source', ''), 'online_store'), p_notes => NULLIF(p_order ->> 'notes', ''),
    p_ad_tracking => p_order -> 'ad_tracking', p_selected_quote_id => NULLIF(p_order ->> 'selected_quote_id', '')::uuid,
    p_shipping_provider => NULLIF(p_order ->> 'shipping_provider', ''), p_tracking_number => NULLIF(p_order ->> 'tracking_number', ''),
    p_user_id => NULLIF(p_order ->> 'user_id', '')::uuid, p_tax_basis => 'exclusive', p_gift_wrapping_fee => COALESCE((p_order ->> 'gift_wrapping_fee')::numeric, 0),
  ...
  INSERT INTO private.uba_redvault_applications AS application (order_id, discount_code_id, merchant_id, quote_version_id, quote_payload_hash, quote_payload, discount_kobo, eligible_subtotal_kobo, status, customer_email, user_id, checkout_key, request_hash)
  VALUES (v_order_id, v_code_id, v_merchant_id, v_quote_version_id, v_hash, p_quote, v_discount_kobo, v_eligible_subtotal_kobo, 'draft', v_customer_email, auth.uid(), v_key, v_request_hash) RETURNING application.id INTO v_application_id;
  ...
  SELECT v_application_id, oi.id, (line.value ->> 'lineId')::integer, unit.value_ordinal, unit.value::bigint,
    oi.product_id, oi.variant_id, oi.condition, COALESCE(oi.variant_attributes, '{}'::jsonb), round(oi.price * 100)::bigint,
    oi.vat_category_code, round(COALESCE(oi.vat_rate, 0) * 100)::integer, 'exclusive'
