...
BEGIN
  IF TG_OP = 'UPDATE'
    ...
    AND EXISTS (
      ...
      WHERE application.order_id = NEW.id
...
ALTER FUNCTION public.approve_and_complete_uba_redvault_payment(uuid, uuid, jsonb)
  RENAME TO approve_and_complete_uba_redvault_payment_legacy_918;
REVOKE ALL ON FUNCTION public.approve_and_complete_uba_redvault_payment_legacy_918(uuid, uuid, jsonb)
...
BEGIN
  ...
  IF v_attempt_state IS DISTINCT FROM 'approved'
    AND (
      ...
      OR lower(COALESCE(v_shipping_status, '')) IN ('canceled', 'cancelled', 'shipped', 'delivered')
    ) THEN
    RAISE EXCEPTION 'redvault_verified_completion_order_state_invalid';
  END IF;

  RETURN public.approve_and_complete_uba_redvault_payment_legacy_918(
    p_transaction_id,
    p_order_id,
    p_verified_evidence
  );
