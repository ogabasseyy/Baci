-- Move REDVAULT verification approval off the service-role client.
-- The user-facing verify flow must not perform payment, order, and inventory
-- mutations while bypassing RLS. These two RPCs now also accept the
-- short-lived scoped storefront route client (authenticated role carrying
-- storefront_order_context = 'route' bound to the REDVAULT merchant). All
-- existing order/attempt/evidence cross-checks are unchanged; only the
-- caller authorization is widened from service_role-only to service_role or
...
BEGIN
  IF (SELECT auth.role()) IS DISTINCT FROM 'service_role'
    ...
    RAISE EXCEPTION 'forbidden: get_uba_redvault_verified_completion_context requires service_role or scoped route context';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.get_uba_redvault_verification_context(uuid, uuid) TO service_role, authenticated;
