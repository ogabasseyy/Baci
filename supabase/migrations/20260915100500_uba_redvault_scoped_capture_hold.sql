-- Move REDVAULT capture holding off the service-role client.
-- The user-facing verify flow must not invoke the capture mutation while
-- bypassing RLS. Both capture entry points now also accept the short-lived
-- scoped storefront route client (authenticated role carrying
-- storefront_order_context = 'route' bound to the REDVAULT merchant), mirroring
-- the scoped verification approval. All capture evidence cross-checks are
-- unchanged; only the caller authorization is widened.
