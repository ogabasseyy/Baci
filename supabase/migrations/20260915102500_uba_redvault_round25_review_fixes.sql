-- Round-25 review fixes: guest REDVAULT attach requires the order's
-- tracking token as order-bound proof (email alone lets anyone who
-- registers the checkout email claim the order), and held captures accept
-- only full-capture recovery (a partial refund on a held attempt strands
-- the remainder in a non-terminal hold the approval RPC then rejects).
-- Round-25 P1: guest attach requires the order's tracking token before
-- transferring ownership. Email equality is not proof the caller created
-- the guest order — anyone who knows the checkout email can register it
-- and Auth issues a session immediately — so the token (the same
-- order-bound proof the guest cancellation path requires) must match.
DROP FUNCTION IF EXISTS public.attach_redvault_guest_application_to_customer(uuid);
