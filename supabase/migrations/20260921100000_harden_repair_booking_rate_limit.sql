-- Serialize the public repair-booking limiter per merchant so concurrent valid
-- bookings cannot both observe a below-limit count, and gate the free-form
-- legacy branch (no quote and no device id) with the same public catalogue
-- check as catalogue-backed bookings. Pickup rows still insert atomically
-- with pickup_payment_status=awaiting_payment.
