-- Require at least ten digits in the repair-booking customer phone so
-- separator-only values like '----------' cannot pass validation and land in
-- public.repairs.customer_phone. Replaces private.create_repair_booking with
-- the tightened check; all other behavior is unchanged.
...
COMMENT ON FUNCTION private.create_repair_booking(
  ...
  'SECURITY DEFINER booking write; serialized per merchant for rate limits, gated on the public repairs catalogue, and pickup rows insert with pickup_payment_status=awaiting_payment so unpaid pickups are never null/null bookable.';
