-- Retain the template-backed free-form booking path: the storefront serves
-- the /repair wizard to merchants on the legacy ogabassey template even
-- without catalogue eligibility (see canUseRepairBooking in
-- repair-page-content.tsx, mirroring OGABASSEY_TEMPLATE_ID in
-- apps/web/src/config/templates.ts), so the catalogue gate keeps an
-- exception for that template instead of rejecting every such submission
-- with catalog_disabled. Replaces private.create_repair_booking with the
-- template exception; all other behavior is unchanged.
...
COMMENT ON FUNCTION private.create_repair_booking(
  ...
  'SECURITY DEFINER booking write; serialized per merchant for rate limits, gated on the public repairs catalogue with eligibility retained for the legacy template-backed free-form path, and pickup rows insert with pickup_payment_status=awaiting_payment so unpaid pickups are never null/null bookable.';
