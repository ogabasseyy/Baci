-- Limit the template-backed booking exception to the free-form path: the
-- storefront serves the /repair wizard to merchants on the legacy ogabassey
-- template even without catalogue eligibility (see canUseRepairBooking in
-- repair-page-content.tsx, mirroring OGABASSEY_TEMPLATE_ID in
-- apps/web/src/config/templates.ts), but catalogue-linked branches (a device
-- id or quote id) still require catalogue eligibility so a stale or hidden
-- option cannot be booked after the feature is disabled or the store is
-- unpublished. Replaces private.create_repair_booking with the narrowed
-- exception; all other behavior is unchanged.
...
BEGIN
  ...
  IF NOT public.repairs_catalog_publicly_enabled(p_merchant_id)
    AND NOT (
      p_device_id IS NULL
      AND p_quote_id IS NULL
      AND EXISTS (
        SELECT 1
        FROM public.merchants AS m
        WHERE m.id = p_merchant_id
          AND m.template_id = 'ogabassey'
      )
    ) THEN
  ...
  IF v_customer_phone !~ '^\+?[0-9[:space:]-]+$'
    OR char_length(regexp_replace(v_customer_phone, '[^0-9]', '', 'g')) < 10 THEN
...
COMMENT ON FUNCTION private.create_repair_booking(
  ...
  'SECURITY DEFINER booking write; serialized per merchant for rate limits, gated on the public repairs catalogue with eligibility retained only for the legacy template-backed free-form path, and pickup rows insert with pickup_payment_status=awaiting_payment so unpaid pickups are never null/null bookable.';
