-- Require publication in the template-backed booking exception: the legacy
-- ogabassey-template free-form path stays open without catalogue eligibility,
-- but an unpublished (draft) store must not accept public bookings through
-- the anon-executable wrapper. The storefront layout renders StoreNotPublished
-- for unpublished slugs, and every other public storefront gate in this schema
-- already requires m.is_published (see
-- 20260711100011_require_published_store_in_repairs_gate.sql). Replaces
-- private.create_repair_booking with the publication-pinned exception; all
...
COMMENT ON FUNCTION private.create_repair_booking(
  ...
  'SECURITY DEFINER booking write; serialized per merchant for rate limits, gated on the public repairs catalogue with eligibility retained only for the legacy template-backed free-form path of published stores, and pickup rows insert with pickup_payment_status=awaiting_payment so unpaid pickups are never null/null bookable.';
