-- Restrict Jumia credential ciphertext reads to managing callers.
-- The RPC is executable by the authenticated role so server routes can call
-- it with the requester's JWT, but read-only staff must not be able to
-- retrieve long-lived credential material directly. Merchant owners,
-- integrations:manage staff, and the service_role worker path keep access;
-- viewers use cached order/item reads that never load grants.
