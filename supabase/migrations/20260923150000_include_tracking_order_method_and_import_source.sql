...
-- Guest success pages need the stored payment method (Pay for Me handoff
-- gate), the imported-order source markers (paid-document resolution),
-- and the provisioned virtual account to render copyable payer
-- instructions (the proof-bound guest branch never loads payment
-- accounts, so the lookup must carry them).
...
COMMENT ON FUNCTION public.get_order_tracking(TEXT, UUID, TEXT, TEXT, TEXT) IS
  'Retrieve order tracking info by token (no email) or by email + order_id/number (legacy), including order item receipt snapshots such as image_url, condition, and variant_name, plus the stored payment method and imported-order source markers.';
