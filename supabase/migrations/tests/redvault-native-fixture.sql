...
CREATE TABLE public.merchants(id uuid PRIMARY KEY,country text,paystack_subaccount_code text);
INSERT INTO public.merchants(id,country,paystack_subaccount_code) VALUES ('6b5cb8a4-5575-456c-b936-8cdfae30db74','NG','ACCT_fixture');
...
CREATE TABLE public.product_variants(id uuid PRIMARY KEY,product_id uuid,price_override numeric,condition text,merchant_id uuid,inventory_tracking_policy text);
...
CREATE TABLE public.merchant_wallets(id uuid PRIMARY KEY DEFAULT extensions.gen_random_uuid(),merchant_id uuid NOT NULL UNIQUE,available_balance numeric(12,2) NOT NULL DEFAULT 0,pending_balance numeric(12,2) NOT NULL DEFAULT 0,total_earned numeric(12,2) NOT NULL DEFAULT 0,total_withdrawn numeric(12,2) NOT NULL DEFAULT 0,upcoming_balance numeric(12,2) NOT NULL DEFAULT 0,upcoming_count integer NOT NULL DEFAULT 0,updated_at timestamptz DEFAULT now());
CREATE TABLE public.merchant_settlements(id uuid PRIMARY KEY DEFAULT extensions.gen_random_uuid(),merchant_id uuid NOT NULL,wallet_id uuid NOT NULL REFERENCES public.merchant_wallets(id),source_type text NOT NULL,source_id uuid,gateway text NOT NULL,gateway_reference text,gross_amount numeric(12,2) NOT NULL,gateway_fee numeric(10,2) NOT NULL DEFAULT 0,platform_fee numeric(10,2) NOT NULL DEFAULT 0,net_amount numeric(12,2) NOT NULL,payment_date timestamptz NOT NULL DEFAULT now(),expected_settlement_date date NOT NULL,actual_settlement_date date,status text NOT NULL DEFAULT 'pending',description text,metadata jsonb NOT NULL DEFAULT '{}'::jsonb,updated_at timestamptz DEFAULT now(),CHECK (status IN ('pending','processing','settled','failed','cancelled')));
CREATE UNIQUE INDEX merchant_settlements_source_reference_active_idx ON public.merchant_settlements(source_type,source_id,gateway_reference) WHERE gateway_reference IS NOT NULL AND status <> 'cancelled';
CREATE TABLE public.wallet_transactions(id uuid PRIMARY KEY DEFAULT extensions.gen_random_uuid(),wallet_id uuid NOT NULL REFERENCES public.merchant_wallets(id),merchant_id uuid NOT NULL,type text NOT NULL,amount numeric(10,2) NOT NULL,balance_after numeric(12,2) NOT NULL,source_type text,source_id uuid,status text NOT NULL DEFAULT 'completed',description text,metadata jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz DEFAULT now(),CHECK (type IN ('credit','debit','withdrawal','payout','refund','adjustment')));
...
CREATE FUNCTION public.calculate_settlement_date(p_gateway text, p_payment_date timestamptz DEFAULT now()) RETURNS date LANGUAGE sql IMMUTABLE AS $$ SELECT CASE p_gateway WHEN 'paystack' THEN (p_payment_date + interval '1 day')::date ELSE p_payment_date::date END $$;
CREATE FUNCTION public.get_or_create_merchant_wallet(p_merchant_id uuid) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ DECLARE v_wallet_id uuid; BEGIN SELECT id INTO v_wallet_id FROM merchant_wallets WHERE merchant_id = p_merchant_id FOR UPDATE; IF v_wallet_id IS NULL THEN INSERT INTO merchant_wallets(merchant_id) VALUES (p_merchant_id) ON CONFLICT (merchant_id) DO NOTHING RETURNING id INTO v_wallet_id; IF v_wallet_id IS NULL THEN SELECT id INTO v_wallet_id FROM merchant_wallets WHERE merchant_id = p_merchant_id; END IF; END IF; RETURN v_wallet_id; END; $$;
